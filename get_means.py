...
def bar_plot_single_metric(fnames, config, nr_trials, base):
    
    all_score = []
    for x in range(0, nr_trials):
        temp = []
        filen = './'+base+'/trial'+str(x+1)+'/'+fnames[0]
        print filen
        ...
        all_score.append([float(temp[1][1]), float(temp[0][1]), float(temp[5][1]), float(temp[4][1]), 
    float(temp[3][1]), float(temp[6][1]), float(temp[2][1]), float(temp[7][1])])
    dynamic_score = np.array(all_score)
    print all_score
    # current order
    # 0         1           2       3           4       5           6       7
    ...
    dynamic_base = []
    for x in range(0, nr_trials):
        filen = './'+base+'/trial'+str(x+1)+'/'+fnames[1]
        temp = []
        with open(filen, 'r') as f:
            lines = f.readlines()
            for line in lines:
        ...
        dynamic_all.append( [ float(temp[1][3]), float(temp[0][3]), float(temp[5][3]), float(temp[4][3]), 
        float(temp[3][3]), float(temp[6][3]), float(temp[2][3]), float(temp[7][3]) ] )

        dynamic_depend.append( [ float(temp[1][6]), float(temp[0][6]), float(temp[5][6]), float(temp[4][6]), 
        float(temp[3][6]), float(temp[6][6]), float(temp[2][6]), float(temp[7][6]) ] )

        dynamic_fires.append( [ float(temp[1][7]), float(temp[0][7]), float(temp[5][7]), float(temp[4][7]), 
        float(temp[3][7]), float(temp[6][7]), float(temp[2][7]), float(temp[7][7]) ] )

        dynamic_victims.append( [ float(temp[1][8]), float(temp[0][8]), float(temp[5][8]), float(temp[4][8]), 
        float(temp[3][8]), float(temp[6][8]), float(temp[2][8]), float(temp[7][8]) ] )

        dynamic_time.append( [ float(temp[9][7]), float(temp[8][7]), float(temp[13][7]), float(temp[12][7]), 
        float(temp[11][7]), float(temp[14][7]), float(temp[10][7]), float(temp[15][7]) ] )

        dynamic_base.append( [ float(temp[9][8]), float(temp[8][8]), float(temp[13][8]), float(temp[12][8]), 
        float(temp[11][8]), float(temp[14][8]), float(temp[10][8]), float(temp[15][8]) ] )
    
    dynamic_all = np.array(dynamic_all)
    dynamic_base = np.array(dynamic_base)
    dynamic_depend = np.array(dynamic_depend)
    dynamic_depend[dynamic_depend==-1] = 0
    dynamic_fires = np.array(dynamic_fires)
    dynamic_time = np.array(dynamic_time)
    dynamic_victims = np.array(dynamic_victims)
    
    all_score = []
    for x in range(0, nr_trials):
        temp = []
        filen = './'+base+'/trial'+str(x+1)+'/'+fnames[2]
        print filen
        with open(filen, 'r') as f:
            lines = f.readlines()
            for line in lines:
        ...
        all_score.append([float(temp[1][1]), float(temp[0][1]), float(temp[5][1]), float(temp[4][1]), 
    float(temp[3][1]), float(temp[6][1]), float(temp[2][1]), float(temp[7][1])])
    static_score = np.array(all_score)
    print all_score
    ######################################################
    ...
    static_base = []
    for x in range(0, nr_trials):
        filen = './'+base+'/trial'+str(x+1)+'/'+fnames[3]
        temp = []
        with open(filen, 'r') as f:
            lines = f.readlines()
            for line in lines:
        ...
        static_all.append( [ float(temp[1][3]), float(temp[0][3]), float(temp[5][3]), float(temp[4][3]), 
        float(temp[3][3]), float(temp[6][3]), float(temp[2][3]), float(temp[7][3]) ] )

        static_depend.append( [ float(temp[1][6]), float(temp[0][6]), float(temp[5][6]), float(temp[4][6]), 
        float(temp[3][6]), float(temp[6][6]), float(temp[2][6]), float(temp[7][6]) ] )

        static_fires.append( [ float(temp[1][7]), float(temp[0][7]), float(temp[5][7]), float(temp[4][7]), 
        float(temp[3][7]), float(temp[6][7]), float(temp[2][7]), float(temp[7][7]) ] )

        static_victims.append( [ float(temp[1][8]), float(temp[0][8]), float(temp[5][8]), float(temp[4][8]), 
        float(temp[3][8]), float(temp[6][8]), float(temp[2][8]), float(temp[7][8]) ] )

        static_time.append( [ float(temp[9][7]), float(temp[8][7]), float(temp[13][7]), float(temp[12][7]), 
        float(temp[11][7]), float(temp[14][7]), float(temp[10][7]), float(temp[15][7]) ] )

        static_base.append( [ float(temp[9][8]), float(temp[8][8]), float(temp[13][8]), float(temp[12][8]), 
        float(temp[11][8]), float(temp[14][8]), float(temp[10][8]), float(temp[15][8]) ] )
    
    static_all = np.array(static_all)
    static_base = np.array(static_base)
    static_depend = np.array(static_depend)
    static_depend[static_depend==-1] = 0
    static_fires = np.array(static_fires)
    static_time = np.array(static_time)
    static_victims = np.array(static_victims)
...
if __name__ == '__main__':
    ...
    if not len(sys.argv) == 8:
        print 'Usage: ./plot_7.py config dynamicScore dynamic1 staticScore static1 nr_of_trials base'
        sys.exit()
    ...
    f3 = sys.argv[4]
    f4 = sys.argv[5]
    name_of_files = [f1, f2, f3, f4]

    print name_of_files   
    bar_plot_single_metric(name_of_files, sys.argv[1], int(sys.argv[6]), sys.argv[7])
